...
//			"--no-keepalive", 15, 			// Disable keepalive use on the connection
    ...
    public  final Curl pure(String curlFromChrome) {
      return opt( Arrays.stream(curlFromChrome
                .replaceFirst("curl ", "")
                .replaceAll("\n  ", "")
                .replaceAll("\n", "")
                .replaceAll("'", "")
                .split("\\\\")
        ).flatMap(x -> Arrays.stream(StringUtils.strip(x).split(" ", 2))).toArray(String[]::new));
...
//						if (!ignoreContentLength) {
    ...
    public static void main(String[] args) {
        ...
        var curl_s ="curl 'http://192.168.18.164:8080/inter-api/auth/login' \\\n" +
                "  -H 'Connection: keep-alive' \\\n" +
                "  -H 'Pragma: no-cache' \\\n" +
                "  -H 'Accept: application/json, text/plain, */*' \\\n" +
                "  -H 'Cache-Control: no-cache, no-store' \\\n" +
                "  -H 'Authorization: Bearer defa8190-36c1-4551-b32b-ae9d8a106623' \\\n" +
                ...
                "  -H 'Origin: http://192.168.18.164:8080' \\\n" +
                "  -H 'Referer: http://192.168.18.164:8080/login/' \\\n" +
                "  -H 'Cookie: suposTicket=defa8190-36c1-4551-b32b-ae9d8a106623' \\\n" +
                "  --data-raw '{\"userName\":\"admin\",\"password\":\"Supos1304@\",\"clientId\":\"ms-content-sample\"}' \\\n" +
                "  --compressed \\\n" +
                "  --insecure";


                var curl= new Curl().pure(curl_s);




        System.out.println(curl.exec(null));
